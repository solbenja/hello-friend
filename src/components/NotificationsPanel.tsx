...
const NotifIcon = ({ type }: { type: string }) => {
  const icons: Record<string, React.ReactElement> = {
    ...
    convert: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>),
    milestone: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>),
  };
  return (
...
const NotificationItem = ({ notif }: { notif: Notification }) => {
  const body = (
    <div className={`flex items-start gap-3 p-3 rounded-xl border transition-all ${notif.link ? 'cursor-pointer hover:border-[#2f2f2f]' : 'cursor-default'} ${notif.read ? "bg-[#0a0a0a] border-[#141414]" : "bg-[#0f0f0f] border-[#1f1f1f]"}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {notif.link && (
          <p className="text-white/80 text-[11px] mt-1 font-mono underline break-all">View Transaction →</p>
        )}
  ...
  if (notif.link) {
    return <a href={notif.link} target="_blank" rel="noopener noreferrer" className="block no-underline">{body}</a>;
  }
  return body;
};
